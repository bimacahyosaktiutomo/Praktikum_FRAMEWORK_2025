<x-layouts.dashboard :title="$title">
    <section class="lg:w-[720px] md:w-[540px] w-screen p-2 bg-slate-50 border-2 border-rose-800 shadow-lg">
        <div class="flex justify-between w-full mb-4">
            <h1 class="font-bold text-xl text-rose-800">Daftar Dosen</h1>
            ...
            <a href="{{ route('lecturer.create') }}" class="inline-flex items-center gap-0.5 text-rose-50 p-1 bg-green-600 cursor-pointer shadow-md duration-100 hover:translate-y-1">
        ...
        <form method="GET" action="{{ route('lecturer.index') }}" class="mb-4 flex gap-2 shadow-lg">
            <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari dosen..."
            ...
            <x-dashboard.tabel>
                ...
                <x-slot:body>
                    @foreach ($data as $index => $lecturer)
                        <tr class="hover:bg-rose-50 odd:bg-slate-100">
                            ...
                            <td class="text-lg text-center">
                                <div class="flex lg:flex-row flex-col justify-center lg:gap-1 text-lg">
                                    <a href="{{ route('lecturer.edit', $lecturer->id) }}" class="text-rose-50 p-1 bg-amber-400 cursor-pointer shadow-md duration-100 hover:translate-y-1">
                        ...
                        <div id="deleteModal-{{ $lecturer->id }}" class="fixed inset-0 hidden items-center justify-center z-50">
                            ...
                            <div class="bg-white p-6 shadow-lg text-center border-2 border-rose-800 w-[90%] max-w-md z-20">
                                <h2 class="text-lg font-semibold text-rose-800 mb-4">Yakin ingin menghapus <span class="underline font-bold text-xl">{{ $lecturer->name }}</span> ?</h2>
                                <div class="flex justify-center gap-3">
                                    ...
                                    <button type="button" onclick="closeModal({{ $lecturer->id }})"
